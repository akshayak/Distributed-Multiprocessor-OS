...
// Saraswathy Hariharakrishnan - 1217141952
...
void client() {
	...
	if (send_file) {
	    printf("Sending file contents from %i for %s\n", port, files[port-1]);
		struct d_type ack;
		int done_flag = 1;
		printf("Sending file from client 3\n");
...
void server() {
	...
	while(1){
		...
		if(message.actionType == 1) //Action type '1' denotes initiation of file transfer.
		{
			if (num_requests > 3) {
			...
			if (st.st_size >= (1024*1024)) {
				send_msg(ports[reply_port], createItem("File size exceeds 1 MB", ports[reply_port], -1, 0));
			}
			else {
				...
				fprintf(fp, "%s", buffer);
				send_msg(ports[reply_port], createItem("fwrite was successful!", ports[reply_port], 0, 0));
				fclose(fp);
			}
		}
